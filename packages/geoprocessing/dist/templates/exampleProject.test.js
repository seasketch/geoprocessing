...
describe("Project metadata settings", () => {
    test("package.json", () => {
        ...
        expect(packageJson.description).toBe("Example project to test geoprocessing project init scripts");
        expect(packageJson.license).toBe("BSD-3-Clause");
        expect(packageJson.author).toBe("Chad Burt");
        expect(packageJson.repository.url).toBe("git+https://github.com/seasketch/example-project.git");
        const geoprocessingJson = JSON.parse(fs.readFileSync("geoprocessing.json").toString());
    ...
    test("function metadata set correctly", () => {
        ...
        expect(GeoprocessingHandler.options.description).toBe("Produces the area of the given sketch");
        expect(GeoprocessingHandler.options.executionMode).toBe("sync");
    });
});
// TODO: check that typescript choice was interpreted correctly
//# sourceMappingURL=exampleProject.test.js.map