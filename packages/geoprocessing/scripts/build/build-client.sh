...
if test -f "../geoprocessing/scripts/build/build-client.sh"; then
  # in monorepo
  # cd ../geoprocessing
  ANALYZE=$ANALYZE NOMINIFY=$NOMINIFY npx tsx ../geoprocessing/scripts/build/buildClient.ts
  ...
  ANALYZE=$ANALYZE NOMINIFY=$NOMINIFY npx tsx node_modules/@seasketch/geoprocessing/scripts/build/buildClient.ts
