BEGIN;
DROP TABLE IF EXISTS osm_land_subdivided;

-- Create new table for subdivided polygons
CREATE TABLE osm_land_subdivided (
  gid serial PRIMARY KEY,
  geom geometry(Polygon, 4326)
);

-- expand multipolygons into polygons
INSERT INTO osm_land_subdivided (geom)
SELECT
  geom
FROM (
  SELECT
...
WITH complex_areas_to_subdivide AS (
    DELETE FROM osm_land_subdivided
...
CREATE INDEX ON osm_land_subdivided USING gist(geom);
