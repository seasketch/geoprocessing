import Cache, { makeCacheKey } from "../src/cache";
import { SeaSketchFeature } from "../src/geometry";
...
describe("cache key generation", () => {
  test("basic hashing", () => {
    const hash1 = makeCacheKey(exampleSketch);
    expect(typeof hash1).toBe("string");
    const hash2 = makeCacheKey({ ...exampleSketch });
    expect(hash1).toBe(hash2);
  });

  test("updatedAt change detection", () => {
    const hash1 = makeCacheKey(exampleSketch);
    const hash2 = makeCacheKey({
      ...exampleSketch,
      properties: {
  ...
  test("invalidate hash by custom properties", () => {
    const hash1 = makeCacheKey(exampleSketch, ["foo"]);
    const hash2 = makeCacheKey(
      {
        ...exampleSketch,
        properties: {
